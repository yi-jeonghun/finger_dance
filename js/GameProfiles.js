class GameProfiles{
	...
	LoadStaticAssets(game_type){
		if(game_type == GAME_TYPE.DDR || game_type == GAME_TYPE.PIANO_TILE){
			...
			this.LoadStaticAssets_PUMP();
		}
	}

	LoadStaticAssets_PUMP(){
		var quarter_x = this.#width / 5;
		...
		{//guide lines
			...
			var vertical_line4 = new DrawLine(this.#ctx, quarter_x*4, 0, quarter_x*4, this.#height, line_width, '#aaa', life_ms);

			base_line_draw_obj.Update();
			vertical_line1.Update();
			vertical_line2.Update();
			vertical_line3.Update();
			vertical_line4.Update();
			...
			window._renderer.AddDrawObject(1, vertical_line4);
		}
		...
		{//empty guide
			...
			var di_c = new DrawImage(this.#ctx, this._atlas._img, 
				this._atlas._img_c_empty.x, this._atlas._img_c_empty.y, this._atlas._img_c_empty.w, this._atlas._img_c_empty.h, 
				((first_x + quarter_x*2) - dw/2), this.#base_line - dh/2, dw, dh, life_ms);
			di_c.Update();
			window._renderer.AddDrawObject(4, di_c);
			...
			var di_r = new DrawImage(this.#ctx, this._atlas._img, 
				...
				((first_x + quarter_x*4) - dw/2), this.#base_line - dh/2, dw, dh, life_ms);
			di_r.Update();
